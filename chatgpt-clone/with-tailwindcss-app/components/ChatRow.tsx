...
function ChatRow({id} : Props) {
    ...
    const [messages] = useCollection(
        collection(db, "users", serverSession?.user?.email!, 'chats', id, "messages")
    )
    ...
    const removeChat = async() => {
        await deleteDoc(doc(db, 'users', serverSession?.user?.email!, 'chats', id));
        router.replace("/");  
    }
  ...
  return <Link href={`/chat/${id}`} className={`chatRow justify-center ${active && 'bg-gray-700/50'}`} >
        ...
        <p className="flex-1 hidden md:inline-flex truncate">
            {/* pulls the latest message from the chat or say New Chat if empty */}
